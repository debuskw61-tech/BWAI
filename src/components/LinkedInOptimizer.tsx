    ...
    <div className="space-y-6 animate-fadeIn" id="linkedin-optimizer-container">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl">
        <div className="flex items-center space-x-3 mb-2">
          ...
          <span className="text-xs font-bold text-blue-400 uppercase tracking-widest">Digital Branding</span>
        </div>
        <h3 className="text-xl font-bold text-slate-100">🔗 LinkedIn Profile Optimizer</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-3xl space-y-4">
            <h4 className="text-sm font-bold text-slate-200 block border-b border-slate-800 pb-2">Atur Profile Saya</h4>

            {/* Suggestions buttons */}
            <div className="space-y-1.5">
              <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block">Gunakan Data Contoh</span>
            ...
            <div className="space-y-4 pt-2">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-300 block uppercase tracking-wider">Target Posisi Pekerjaan</label>
                <input
                  type="text"
                  value={targetRole}
                  onChange={(e) => setTargetRole(e.target.value)}
                  placeholder="Contoh: Frontend Developer, Social Media Manager"
                  className="w-full bg-slate-950 border border-slate-800 text-slate-100 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-300 block uppercase tracking-wider">Tulis Riwayat Singkat / Pengalaman Anda</label>
                <textarea
                  value={experiences}
                  ...
                  className="w-full h-32 bg-slate-950 border border-slate-800 text-slate-100 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 leading-relaxed resize-none"
                />
              </div>
            </div>
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold rounded-xl text-sm flex items-center justify-center space-x-2 cursor-pointer transition shadow-lg shadow-indigo-600/10"
            >
              {loading ? (
                <>
        ...
        <div className="lg:col-span-7 space-y-6">
          {!result && !loading && (
            <div className="bg-slate-900/50 border border-slate-800/80 rounded-3xl p-8 text-center flex flex-col items-center justify-center min-h-[400px]">
              <div className="p-3 bg-slate-900 rounded-full border border-slate-800 mb-3 text-blue-400">
                <Linkedin className="w-8 h-8 animate-pulse" />
              </div>
              <h4 className="text-slate-200 font-bold mb-1">Branding Siap Ditulis Ulang</h4>
          ...
          {loading && (
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 flex flex-col items-center justify-center min-h-[400px] space-y-4">
              <div className="relative">
                ...
                <Linkedin className="w-6 h-6 text-indigo-400 absolute top-5 left-5 animate-bounce" />
              </div>
              <div className="text-center space-y-2">
                <h4 className="text-slate-200 font-bold">Sedang Merancang Personal Branding Terbaik...</h4>
          ...
          {result && (
            <div className="space-y-6 animate-fadeIn" id="linkedin-result-display">
              ...
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-3xl space-y-3">
                ...
                <p className="text-[11px] text-slate-400">Pilih salah satu model headline di bawah ini untuk dipasang di bagian headline utama akun LinkedIn Anda:</p>
                
                <div className="space-y-2 pt-1">
                  {result.headlineOptions.map((headline, idx) => (
                    <div key={idx} className="bg-slate-950 border border-slate-850 p-3.5 rounded-xl flex justify-between items-center group relative gap-3">
                      <span className="text-slate-200 text-xs md:text-sm font-medium leading-relaxed">
                        {headline}
                      </span>
                      <button
                        onClick={() => handleCopyText(headline, "headline", idx)}
                        className="p-2 bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-slate-100 rounded-lg shrink-0 cursor-pointer border border-slate-800 transition"
                        title="Copy headline"
                      >
              ...
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-3xl space-y-3">
                <div className="flex justify-between items-center border-b border-slate-850 pb-2">
                  ...
                  <button
                    ...
                    className="text-xs bg-slate-950 border border-slate-800 text-indigo-400 hover:text-indigo-200 px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition cursor-pointer"
                  >
                    {copiedSummary ? (
                      <>
                ...
                <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl text-slate-300 text-xs md:text-sm leading-relaxed whitespace-pre-wrap font-sans">
                  {result.summary}
                </div>
              </div>
              ...
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-3xl space-y-2">
                <span className="text-[10px] text-slate-400 uppercase font-bold tracking-widest flex items-center gap-1.5">
                  <Key className="w-4 h-4" /> Keyword Rekomendasi (SEO Matchmaking)
                </span>
                ...
                <div className="flex flex-wrap gap-2 pt-1.5">
                  {result.keyKeywords.map((tag, idx) => (
                    <span key={idx} className="bg-indigo-950/20 border border-indigo-500/20 text-indigo-300 text-xs px-3 py-1.5 rounded-xl font-medium shadow-xs">
                      #{tag}
                    </span>
                  ))}
                </div>
